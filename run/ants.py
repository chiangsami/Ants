...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        adjacent_place=self.place
        distance=0
        while adjacent_place.is_hive==False:
            if adjacent_place.bees and self.lower_bound<=distance<=self.upper_bound:
                return random_bee(adjacent_place.bees)
            adjacent_place=adjacent_place.entrance
            distance+=1
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.turns_to_chew>0:
            ...
            chomped_bee=random_bee(self.place.bees)
            chomped_bee.reduce_health(chomped_bee.health)  
...
# END Problem 12
    ...
    # BEGIN Problem 12
    implemented = True
    QueenAnt= True   # Change to True to view in the GUI
    # END Problem 12
        ...
        while behind is not None: 
            if behind.ant is not None: 
                ...
                if behind.ant.is_container and behind.ant.ant_contained is not None:
                    if behind.ant.ant_contained.is_doubled == False:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        elif self.health > 0 and destination is not None and (self.is_slowed==0 or gamestate.time%2==0):
